...
function validateItemInput(input, { partial = false } = {}) {
  ...
  const allowed = ['name', 'unit_price_crc', 'goal_quantity', 'active', 'sort_order'];
  ...
  if (has('sort_order')) {
    if (!Number.isInteger(input.sort_order) || Math.abs(input.sort_order) > 100000) {
      throw new DomainError('El orden debe ser un número entero.', { field: 'sort_order' });
    }
    fields.sort_order = input.sort_order;
  }

  return fields;
}

// New items go to the end of the list unless a sort_order is given.
export async function createItem(input) {
  const fields = validateItemInput(input);
  const result = await db.query({
    name: 'createItem',
    text: `
      INSERT INTO items (name, unit_price_crc, goal_quantity, active, sort_order)
      VALUES ($1, $2, $3, COALESCE($4, true),
              COALESCE($5, (SELECT COALESCE(MAX(sort_order), 0) + 1 FROM items)))
      RETURNING id
    `,
    values: [
      fields.name,
